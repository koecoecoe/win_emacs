...
;;(add-to-list 'default-frame-alist '(font . "HGｺﾞｼｯｸM"))
(add-to-list 'initial-frame-alist '(font . "HGｺﾞｼｯｸM"))
...
;; 日付時間挿入
(defun insert-current-daytime()
  (interactive)
  (insert (format-time-string "%Y/%m/%d %H:%M:%S \n" (current-time))))

(define-key global-map [f5] `insert-current-daytime)
...
(define-key global-map [?\C-;] `insert-current-time)

;;trampの設定（puty）
(setenv "PATH" (concat "C:\\Users\masaki_hashimoto\Documents\putty-0.60-JP_Y-2007-08-06\plink.exe" ";" (getenv "PATH")))

;;フォント設定

(set-fontset-font "fontset-standard"
   		  'ascii
   		  (font-spec :family "HGｺﾞｼｯｸM" :size 11) nil 'prepend)  ;; ここでサイズを指定
